...
if (hallInfo !== "null") {
  // если нам сервер прислал разметку то делаем следующиее
...
let buyingInfoDescription = document.getElementsByClassName(
  "buying__info-description"
)[0]; //меняем информаци о сеансе, берём данные которые сохраняли в sessionStroage, при запросе на сервер
buyingInfoDescription.getElementsByClassName(
  "buying__info-title"
)[0].textContent = sessionStorage.getItem("data-film-name");
buyingInfoDescription.getElementsByClassName(
  "buying__info-start"
)[0].textContent = `Начало сеанса: ${sessionStorage.getItem(
  "data-seance-time"
)}`;
buyingInfoDescription.getElementsByClassName(
  "buying__info-hall"
)[0].textContent = sessionStorage.getItem("data-hall-name");

document.getElementsByClassName(
  "conf-step__legend-value price-standart"
)[0].textContent = sessionStorage.getItem("data-price-standart");
document.getElementsByClassName(
  "conf-step__legend-value price-vip"
)[0].textContent = sessionStorage.getItem("data-price-vip");

let wrapperChildren = wrapper.children;

for (let i = 0; i < wrapper.childElementCount; i++) {
  //перебираем каждое место и назначаем на него функцию клика
  ...
  for (let j = 0; j < chairs.length; j++) {
    ...
    chairs[j].addEventListener("mouseenter", function () {
      // смена указателя при наведении на место
...
buyButton.onclick = function () {
  // отправляем выбранное место
  let chairsSelected = Array.from(
    wrapper.getElementsByClassName("conf-step__chair_selected")
  ); // выбранные места
  if (chairsSelected.length) {
    ...
    chairsSelected.forEach((e) => {
      //сортируем какое место к какому ряду
    ...
    sessionStorage.setItem(
      sessionStorage.getItem("data-seance-id"),
      JSON.stringify(saveCurrentBuy)
    ); // сохраняем

    let data = `event=sale_add&timestamp=${sessionStorage.getItem(
      "data-seance-timestamp"
    )}&hallId=${sessionStorage.getItem(
      "data-hall-id"
    )}&seanceId=${sessionStorage.getItem("data-seance-id")}&hallConfiguration=${
      wrapper.innerHTML
    }`;
    request(() => {
      chairsSelected.forEach((e) => {
...
buyingInfoHint.addEventListener("touchend", function (event) {
  ...
  if (now - lastTouchEnd <= 300) {
    ...
    let confStepWrapper =
      document.getElementsByClassName("conf-step__wrapper")[0]; // увеличиваем
    let confStepLegend =
      document.getElementsByClassName("conf-step__legend")[0]; //добавляем блоку отступ от увеличенного
