function getHallConfig(hallConfig) {
  ...
  chairList.forEach((c) =>
    c.addEventListener("click", (event) => {
      ...
      if (
        !c.classList.contains("conf-step__chair_disabled") &&
        !c.classList.contains("conf-step__chair_taken")
      ) {
  ...
  acceptinButton.addEventListener("click", (event) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      let spanPlaces = Array.from(
        rows[i].querySelectorAll(".conf-step__chair")
      );
...
Array.from(
  document.querySelectorAll(".conf-step__row .conf-step__chair_selected")
).forEach((s) => s.classList.remove(".conf-step__chair_selected"));
...
document.querySelector(".price-standart").textContent =
  window.localStorage.getItem("priceStandart");
document.querySelector(".price-vip").textContent =
  window.localStorage.getItem("priceVip");

requestApi(
  `event=get_hallConfig&timestamp=${timestamp}&hallId=${hallId}&seanceId=${seanceId}`,
  getHallConfig
);
