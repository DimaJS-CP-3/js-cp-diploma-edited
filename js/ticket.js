...
wrap.getElementsByClassName("ticket__details ticket__title")[0].textContent =
  tickets.currentBuy["data-film-name"]; //имя фильма

let ticketChairs = wrap.getElementsByClassName(
...
wrap.getElementsByClassName("ticket__details ticket__hall")[0].textContent =
  tickets.currentBuy["data-hall-name"].substr(
    tickets.currentBuy["data-hall-name"].length - 1
  ); // номер зала
wrap.getElementsByClassName("ticket__details ticket__start")[0].textContent =
  tickets.currentBuy["data-seance-time"]; //начало сеанса
...
let textQr = `Фильм: ${tickets.currentBuy["data-film-name"]}\nРяд/Место: ${
  ticketChairs.textContent
}\nЗал: ${tickets.currentBuy["data-hall-name"].substr(
  tickets.currentBuy["data-hall-name"].length - 1
