function startSetInfo() {
  if (!localStorage.length) {
    // если файлов нет , обращаемся на сервер
    request((info) => {
    ...
    if (currentDate.getDate() === date.getDate()) {
      // если дата сохранения сегодняшняя , то получаем информацию из локальных файлов. (currentDate из setDate.js)
      let info = JSON.parse(localStorage.getItem("info")); // переводим в объект и устанавливаем фильмы
      setFilms(info, selectDay);
    } else {
      // делаем запрос т.к. файлы устарели
      request((info) => {
        localStorage.setItem("info", info); // сохранили информацию с сервера
        localStorage.setItem("date", new Date()); // записали дату сохранения
        setFilms(JSON.parse(info), selectDay); // устанавливаем фильмы
...
function setFilms(info, date) {
  // принимает информацию которая приходит с сервера 1 запросом либо из localStroage, date - конкретный день и последующие дни , является Date.
  ...
  )[0].cloneNode(true); // копия блока сеанса
  ...
  info.films.result.forEach((element) => {
    //добавляем секции с новой информацией
    ...
    Array.from(
      ...
      " " +
      declension(Number(element.film_duration), ["минута", "минуты", "минут"]);
    Array.from(
    ...
    Object.keys(sortedHalls).forEach((key) => {
      ...
      hall.getElementsByTagName("h3")[0].textContent =
        key.substring(0, 3) + " " + key.substring(3); // название Зала(Зал1 разделяем на Зал 1)

      sortedHalls[key].forEach((time) => {
        // перебираем время на конкретного фильма в конкретном зале
        ...
        let data = `event=get_hallConfig&timestamp=${refHall.getAttribute(
          "data-seance-timestamp"
