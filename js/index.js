...
allDay.forEach((d) =>
  d.addEventListener("click", (event) => {
    ...
    const movieSeancesTimeNotActive = document.querySelectorAll(
      ".movie-seances__time"
    );
...
function getHallsFilmsSeanses(data) {
  ...
  cinema.films.forEach((film) => {
    ...
    cinema.halls.forEach((hall) => {
      let seances = cinema.seances.filter(
        (seance) =>
          seance.seance_hallid == hall.hall_id && seance.seance_filmid == filmId
      );
      if (seances.length > 0) {
        seancesHTML += `
  ...
  movieSeances.forEach((s) =>
    s.addEventListener("click", (event) => {
      event.preventDefault();
      let currentTimestemp =
        s.dataset.seanceStart * 60 +
        Number(window.localStorage.getItem("timestemp"));
      ...
      let hallConfig = cinema.halls.find(
        (h) => h.hall_id === s.dataset.hallId
      ).hall_config;
