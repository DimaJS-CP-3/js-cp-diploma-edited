...
function updateHtmlMain(serverResponse) {
  ...
  const arrHalls = response.halls.result.filter(
  ...
  const selectedDayTimeStamp = document.querySelector(
    "nav .page-nav__day_chosen"
  ).dataset.dayTimeStamp;
  ...
  arrFilms.forEach((elementFilm) => {
    ...
    arrHalls.forEach((elementHall) => {
      ...
      const arrSeancesCurrentFilmAndHall = arrSeances.filter(
        (seance, index, array) => {
          return (
      ...
      const hallNameText = `${elementHall.hall_name.slice(
        0,
        3
      )} ${elementHall.hall_name.slice(3).trim()}`;
      ...
      if (arrSeancesCurrentFilmAndHall.length) {
        ...
        const mooviSeancesList = movieSection?.querySelector(
        ...
        arrSeancesCurrentFilmAndHall.forEach((elementSeance) => {
          const seanceTimeStamp =
            +selectedDayTimeStamp + +elementSeance.seance_start * 60 * 1000;
